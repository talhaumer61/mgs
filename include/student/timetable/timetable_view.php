...
<section class="panel panel-featured panel-featured-primary appear-animation" data-appear-animation="fadeInRight" data-appear-animation-delay="100">';
	...
	$sqllms_std	= $dblms->querylms("SELECT id_class, id_section, id_session
									FROM ".STUDENTS." 
	...
	$sqllms	= $dblms->querylms("SELECT t.id, t.status, t.id_session, t.id_class, t.id_section, t.id_campus,
									...
									WHERE t.id_class = '".$values_std['id_class']."' AND t.id_section = '". $values_std['id_section']."'
									AND id_session = '".$values_std ['id_session']."' AND t.id_campus = '".$_SESSION['userlogininfo']['LOGINCAMPUS']."' 
									LIMIT 1");
	$rowsvalues = mysqli_fetch_array($sqllms);
	if(mysqli_num_rows($sqllms) > 0){
		echo '
		<header class="panel-heading">
			<h2 class="panel-title"><i class="fa fa-clock-o"></i> Daily Class Routiene of <b>'.$rowsvalues['class_name'].'</b> (<b> '.$rowsvalues['section_name'].'</b>)</h2>
		</header>
					...
					<tbody>
						<tr>
							...
							while($rowsub = mysqli_fetch_array($sqllmssub)){ 
								$periods[] = $rowsub;
								echo'<th style="text-align: center;">'.$rowsub['period_name'].' ('.$rowsub['period_timestart'].' - '.$rowsub['period_timeend'].')</th>';
							}
							echo'
						</tr>
						<tr>';
							for($i=1; $i<=7; $i++){
								...
								foreach($periods as $itemperiod) {  
									...
									$sqllmsdetail	= $dblms->querylms("SELECT d.id, d.day, d.id_subject, d.id_room, d.id_period, d.id_teacher, d.id_setup,
																	...
																	AND id_session = '".$values_std ['id_session']."' 
		...
		echo'<h2 class="panel-body text-center font-bold text text-danger mt-none">No Record Found</h2>';
	}
//}
echo'
</section>';
