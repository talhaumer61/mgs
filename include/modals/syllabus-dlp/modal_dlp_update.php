...
//---------------------------------------------------------
	$sqllms	= $dblms->querylms("SELECT  syllabus_id, syllabus_status, syllabus_term, id_session, syllabus_file, file_thumbnail, id_month, id_week, id_class, id_subject, note
								   FROM ".SYLLABUS."
	...
	<input type="hidden" name="syllabus_id" id="syllabus_id" value="'.cleanvars($_GET['id']).'">
		...
		<div class="panel-body">
						...
						<select class="form-control" required title="Must Be Required" data-plugin-selectTwo data-width="100%" data-minimum-results-for-search="Infinity" id="id_week" name="id_week">
							<option value="">Select</option>';
								foreach($weeks as $week) {
									echo '<option value="'.$week['id'].'"'; if($rowsvalues['id_week'] == $week['id']){ echo 'selected';} echo'>'.$week['name'].'</option>';
								}
						echo '
						</select>
					</div>
			...
			<div class="form-group mb-md">
				...
				<div class="col-md-9">
					<textarea data-plugin-summernote class="form-control summernote summernoteEx" name="note" id="note">'.$rowsvalues['note'].'</textarea>
				</div>
			</div>
			<div class="form-group">
