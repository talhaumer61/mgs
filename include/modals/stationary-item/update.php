...
if(($_SESSION['userlogininfo']['LOGINAFOR'] == 1 && $_SESSION['userlogininfo']['LOGINTYPE'] == 1) || ($_SESSION['userlogininfo']['LOGINTYPE'] == '1' && in_array('34', $_SESSION['userlogininfo']['PERMISSIONS'])) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '34', 'edit' => '1'))) {
	$sqllms	= $dblms->querylms("SELECT item_id, item_status, id_cat, item_name, item_code, headoffice_price, school_price, std_price, item_detail, id_supplier
								   FROM ".INVENTORY_ITEMS."
								   WHERE item_id = '".cleanvars($_GET['id'])."' LIMIT 1");
	$rowsvalues = mysqli_fetch_array($sqllms);
	if($rowsvalues['id_supplier'] == 1)	{
		$disabled = 'disabled';
	} else {
		$disabled = '';
	}
	...
	<section class="panel panel-featured panel-featured-primary">
		<form action="stationary_item.php" class="form-horizontal" id="form" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			...
			<input type="hidden" name="id_supplier" id="id_supplier" value="'.$rowsvalues['id_supplier'].'">
			...
			<div class="panel-body">
				<div class="form-group mt-sm">
					...
					<div class="col-md-9">
						<input type="text" class="form-control" name="item_name" id="item_name" required title="Must Be Required" value="'.$rowsvalues['item_name'].'" '.$disabled.'/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-md-3 control-label">Category <span class="required">*</span></label>
					<div class="col-md-9">
						<select class="form-control" required title="Must Be Required" data-plugin-selectTwo data-width="100%" data-minimum-results-for-search="Infinity" name="id_cat" '.$disabled.'>
				...
				<div class="form-group">
					...
					<div class="col-md-9">
						<input class="form-control" rows="3" id="item_code" name="item_code" value="'.$rowsvalues['item_code'].'" '.$disabled.'/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-md-3 control-label">Head Office Price <span class="required">*</span></label>
					<div class="col-md-9">
						<input type="number" class="form-control" rows="3" name="headoffice_price" id="headoffice_price" value="'.$rowsvalues['headoffice_price'].'" '.$disabled.'/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-md-3 control-label">Sale Price <span class="required">*</span></label>
					<div class="col-md-9">
						<input type="number" class="form-control" rows="3" name="school_price" id="school_price" value="'.$rowsvalues['school_price'].'"/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-md-3 control-label">School Price <span class="required">*</span></label>
					<div class="col-md-9">
						<input type="number" class="form-control" rows="3" name= "std_price" id="std_price" value="'.$rowsvalues['std_price'].'"/>
					</div>
				</div>
				<div class="form-group">
					<label class="col-md-3 control-label">Item Detail <span class="required">*</span></label>
					<div class="col-md-9">
						<textarea class="form-control" rows="3" id="item_detail" name="item_detail" '.$disabled.'>'.$rowsvalues['item_detail'].'</textarea>
					</div>
				</div>
				<div class="form-group">
