...
//---------------------------------------------------------
	$sqllms	= $dblms->querylms("SELECT  s.syllabus_id, s.syllabus_status, s.syllabus_term, s.id_session, s.syllabus_file, s.file_thumbnail, s.id_class, s.id_subject, s.note
								   FROM ".SYLLABUS." s  
								   WHERE s.syllabus_id = '".cleanvars($_GET['id'])."' AND s.syllabus_type = '1' LIMIT 1");
	...
	<input type="hidden" name="syllabus_id" id="syllabus_id" value="'.cleanvars($_GET['id']).'">
		...
		<div class="panel-body">
			...
			<div class="form-group">
				...
				<div class="col-md-9">
					<input type="file" class="form-control" name="syllabus_file" accept=".docx, .ppt, .pdf" id="syllabus_file" />
				</div>
			</div>
			<div class="form-group">
