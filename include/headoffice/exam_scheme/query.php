...
if(isset($_POST['submit_scheme'])) { 
	$sqllmscheck  = $dblms->querylms("SELECT id
										FROM ".EXAM_DOWNLOADS." 
										WHERE id_type = '3' AND id_exam = '".cleanvars($_POST['id_exam'])."' AND id_month = '".cleanvars($_POST['id_month'])."' 
										AND id_class = '".cleanvars($_POST['id_class'])."' AND id_session = '".cleanvars($_POST['id_session'])."' LIMIT 1");
...
//------------------------------------------------
	...
	//--------------------------------------
		$originalImage	= $img_dir.to_seo_url(cleanvars($_POST['id_session'].'-'.$_POST['id_class'].'-'.$_POST['id_month'])).'_'.$id.".".($extension);
		$img_fileName	= to_seo_url(cleanvars($_POST['id_session'].'-'.$_POST['id_class'].'-'.$_POST['id_month'])).'_'.$id.".".($extension);
	//--------------------------------------
