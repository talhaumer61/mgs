...
<section class="panel panel-featured panel-featured-primary">
	<header class="panel-heading">';
		if(($_SESSION['userlogininfo']['LOGINTYPE']  == 1) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '59', 'add' => '1'))){ 
			echo'
			<a href="#make_dlp" class="modal-with-move-anim btn btn-primary btn-xs pull-right">
			<i class="fa fa-plus-square"></i> Make DLP
			</a>';
		}
		echo'
		<h2 class="panel-title"><a href="resource_pack.php"><i class="fa fa-home"></i> Resource Pack</a> / Syllabus Dlp</h2>
	</header>
	<div class="panel-body">
						...
						<div class="card card-file">
							<div class="dropdown">
								...
								<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">';
									if($rowsvalues['note']){
										echo'<li><a href="#show_modal" class="modal-with-move-anim-pvs dropdown-item" onclick="showAjaxModalZoom(\'include/modals/syllabus-dlp/modal_dlp_details.php?id='.$rowsvalues['syllabus_id'].'\');"><i class="fa fa-info-circle"></i> View Details</a></li>';
									}
									if($rowsvalues['syllabus_file']){
										echo'<li><a href="uploads/dlp/'.$rowsvalues['syllabus_file'].'" class="dropdown-item" target="_blank"><i class="fa fa-eye"></i> View File </a></li>';
										echo'<li><a href="uploads/dlp/'.$rowsvalues['syllabus_file'].'" download="'.$rowsvalues['session_name'].'-'.get_monthtypes($rowsvalues['id_month']).'-'.get_week($rowsvalues['id_week']).'-'.$rowsvalues['class_name'].'-'.$rowsvalues['subject_name'].'" class="dropdown-item"><i class="fa fa-download"></i> Download</a></li>';
									}
									if(($_SESSION['userlogininfo']['LOGINTYPE']  == 1) ||  Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '59', 'edit' => '1'))){ 
										echo'<li><a href="#show_modal" class="modal-with-move-anim-pvs dropdown-item" onclick="showAjaxModalZoom(\'include/modals/syllabus-dlp/modal_dlp_update.php?id='.$rowsvalues['syllabus_id'].'\');"><i class="fa fa-edit"></i> Edit</a></li>';
									}
									if(($_SESSION['userlogininfo']['LOGINTYPE']  == 1) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '59', 'delete' => '1'))){ 
										echo'<li><a href="#" class="dropdown-item" onclick="confirm_modal(\'syllabus_dlp.php?deleteid='.$rowsvalues['syllabus_id'].'\');"><i class="fa fa-trash"></i> Delete</a></li>';
									}
									echo'
								</div>
							</div>
							<div class="card-file-thumb">
								<img src="'.(!empty($rowsvalues['file_thumbnail']) ? 'uploads/dlp/thumbnail/'.$rowsvalues['file_thumbnail'].'' : 'assets/images/files/'.$icon.'.png').'" width="100" height="100">
							</div>
							<div class="card-body">
								<h6>'.$rowsvalues['class_name'].' ('.$rowsvalues['session_name'].')</h6>
								<h6>
									Term: <span class="d-none d-sm-inline" >'.get_term($rowsvalues['syllabus_term']).' </span>
									| File Type: <span class="d-none d-sm-inline" >'.$icon.' </span>
								</h6>
								<h6>
									<span class="d-none d-sm-inline" >'.get_monthtypes($rowsvalues['id_month']).' </span>
									 | <span class="d-none d-sm-inline" >'.get_week($rowsvalues['id_week']).' </span>
								</h6>
							</div>
		...
		// 				if($rowsvalues['note']){
		// 					echo'<a href="#show_modal" class="modal-with-move-anim-pvs btn btn-warning btn-xs" onclick="showAjaxModalZoom(\'include/modals/syllabus-dlp/modal_dlp_details.php?id='.$rowsvalues['syllabus_id'].'\');"><i class="glyphicon glyphicon-comment"></i></a>';
		// 				}
		// 				echo'
		...
		// 				if(($_SESSION['userlogininfo']['LOGINTYPE']  == 1) ||  Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '59', 'edit' => '1'))){ 
		// 				echo'
		// 					<a href="#show_modal" class="modal-with-move-anim-pvs btn btn-primary btn-xs" onclick="showAjaxModalZoom(\'include/modals/syllabus-dlp/modal_dlp_update.php?id='.$rowsvalues['syllabus_id'].'\');"><i class="glyphicon glyphicon-edit"></i></a>';
		// 				}
		// 				if(($_SESSION['userlogininfo']['LOGINTYPE']  == 1) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '59', 'delete' => '1'))){ 
		// 				echo'
