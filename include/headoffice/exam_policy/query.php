...
if(isset($_POST['submit_policy'])) { 
	$sqllmscheck  = $dblms->querylms("SELECT id
										FROM ".EXAM_DOWNLOADS." 
										WHERE id_type = '2' AND id_exam = '".cleanvars($_POST['id_exam'])."' 
...
//------------------------------------------------
	...
	//--------------------------------------
		$originalImage	= $img_dir.to_seo_url(cleanvars($_POST['id_session'].'-'.$_POST['id_exam'])).'_'.$id.".".($extension);
		$img_fileName	= to_seo_url(cleanvars($_POST['id_session'].'-'.$_POST['id_exam'])).'_'.$id.".".($extension);
	//--------------------------------------
...
//--------------------------------------
	if($sqllms) { 
//--------------------------------------
	$remarks = 'Assessment Policy Added ID: '.$id.'" detail';
...
//--------------------------------------										  
	...
	//--------------------------------------
		...
		$img_dir 	= 'uploads/assessment_downloads/';
	//--------------------------------------
		$originalImage	= $img_dir.to_seo_url(cleanvars($_POST['id_class'])).'_'.$id.".".($extension);
		$img_fileName	= to_seo_url(cleanvars($_POST['id_class'])).'_'.$id.".".($extension);
	//--------------------------------------
		if(in_array($extension , array('pdf','ppt', 'docx'))) { 
	//--------------------------------------
			$sqllmsupload  = $dblms->querylms("UPDATE ".EXAM_DOWNLOADS."
