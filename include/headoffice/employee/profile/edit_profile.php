...
$sqllms	= $dblms->querylms("SELECT e.emply_id, e.emply_status, e.emply_regno, e.emply_name, e.id_dept, 
								...
								e.emply_address, e.emply_phone, e.emply_email, e.emply_photo, e.id_campus,
								e.is_de, e.is_ad,
	...
	<form action="#" class="form-horizontal validate" enctype="multipart/form-data" method="post" accept-charset="utf-8">
		...
		<fieldset class="mt-lg">
			...
			<div class="form-group">
				...
				<div class="col-md-8">
					<input type="text" class="form-control" required name="emply_regno" readonly id="emply_regno" value="'.$rowsvalues['emply_regno'].'"/>
				</div>
			</div>
			<div class="form-group">
			...
			if($rowsvalues['id_campus'] == $_SESSION['userlogininfo']['LOGINCAMPUS']){ 
				echo'
				<div class="form-group mt-md mb-sm">
					<label class="col-md-3 control-label">AD/DE </label>
					<div class="col-md-9">
						<div class="checkbox-custom checkbox-inline">
							<input type="checkbox" id="is_ad" name="is_ad" value="1" '.($rowsvalues['is_ad'] == 1 ? 'checked' : '').'>
							<label for="checkboxExample&quot;">Is ADE? </label>
						</div>
						<div class="checkbox-custom checkbox-inline">
							<input type="checkbox" id="is_de" name="is_de" value="1" '.($rowsvalues['is_de'] == 1 ? 'checked' : '').'>
