...
<section class="panel panel-featured panel-featured-primary">
	<header class="panel-heading">';
		...
		echo'
		<h2 class="panel-title"><a href="resource_pack.php"><i class="fa fa-home"></i> Resource Pack</a> / Monthly Assessments</h2>
	</header>
	<div class="panel-body">
						...
						<div class="card card-file">
							<div class="dropdown">
								...
								<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">';
									if($rowsvalues['note']){
										echo'<li><a href="#show_modal" class="modal-with-move-anim-pvs dropdown-item" onclick="showAjaxModalZoom(\'include/modals/monthly-assessment/modal_assessment_details.php?id='.$rowsvalues['syllabus_id'].'\');"><i class="fa fa-info-circle"></i> View Details</a></li>';
									}
									if($rowsvalues['syllabus_file']){
										echo'<li><a href="uploads/monthly_assessments/'.$rowsvalues['syllabus_file'].'" class="dropdown-item" target="_blank"><i class="fa fa-eye"></i> View File </a></li>';
										echo'<li><a href="uploads/monthly_assessments/'.$rowsvalues['syllabus_file'].'" download="'.$rowsvalues['session_name'].'-'.get_monthtypes($rowsvalues['id_month']).'-'.$rowsvalues['class_name'].'-'.$rowsvalues['subject_name'].'" class="dropdown-item"><i class="fa fa-download"></i> Download</a></li>';
									}
									if(($_SESSION['userlogininfo']['LOGINTYPE']  == 1) ||  Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '57', 'edit' => '1'))){ 
										echo'<li><a href="#show_modal" class="modal-with-move-anim-pvs dropdown-item" onclick="showAjaxModalZoom(\'include/modals/monthly-assessment/modal_assessment_update.php?id='.$rowsvalues['syllabus_id'].'\');"><i class="fa fa-edit"></i> Edit</a></li>';
									}
									if(($_SESSION['userlogininfo']['LOGINTYPE']  == 1) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '57', 'delete' => '1'))){ 
										echo'<li><a href="#" class="dropdown-item" onclick="confirm_modal(\'monthly_assessment.php?deleteid='.$rowsvalues['syllabus_id'].'\');"><i class="fa fa-trash"></i> Delete</a></li>';
									}
									echo'
								</div>
							</div>
							<div class="card-file-thumb">
								<img src="'.(!empty($rowsvalues['file_thumbnail']) ? 'uploads/monthly_assessments/thumbnail/'.$rowsvalues['file_thumbnail'].'' : 'assets/images/files/'.$icon.'.png').'" width="100" height="100">
							</div>
							<div class="card-body">
		...
		// 				<td  width="160" class="center">';
		// 					if($rowsvalues['note']){
		// 						echo'<a href="#show_modal" class="modal-with-move-anim-pvs btn btn-warning btn-xs ml-xs" onclick="showAjaxModalZoom(\'include/modals/monthly-assessment/modal_assessment_details.php?id='.$rowsvalues['syllabus_id'].'\');"><i class="glyphicon glyphicon-comment"></i></a>';
		// 					}
		// 					echo'
