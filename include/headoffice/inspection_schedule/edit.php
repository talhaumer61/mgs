...
if(($_SESSION['userlogininfo']['LOGINTYPE']  == 1) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '85', 'edit' => '1'))) { 
	...
	if(isset($_GET['id'])){
		$sqllmsSche  = $dblms->querylms("SELECT schedule_id, schedule_month, schedule_approval, id_adde
										FROM ".INSPECTION_SCHEDULE." 
										WHERE schedule_id = '".cleanvars($_GET['id'])."' AND is_deleted != '1' LIMIT 1");
		if(mysqli_num_rows($sqllmsSche) > 0) {
			...
			$sqllmsCampus = $dblms->querylms("SELECT c.campus_id, c.campus_name, c.campus_code, t.city_name, d.dist_name
												...
												AND c.is_deleted		= '0' 
												AND (c.id_ad = '".$valueSch['id_adde']."' OR c.id_de = '".$valueSch['id_adde']."')
							...
							<div class="panel-body">';
								while($valueCampus = mysqli_fetch_array($sqllmsCampus)) {
									...
									$sqllmsDet  = $dblms->querylms("SELECT purposed_date
																	...
																	AND id_campus = '".$valueCampus['campus_id']."' LIMIT 1");
								...
								echo'
								<input type="hidden" name="id_adde" value="'.$valueSch['id_adde'].'">
