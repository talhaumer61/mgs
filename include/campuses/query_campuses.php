...
if(isset($_POST['submit_campus'])) { 
	$sqllmscheck  = $dblms->querylms("SELECT campus_name  
										...
										AND campus_name = '".cleanvars($_POST['campus_name'])."' LIMIT 1");
...
//--------------------------------------
	...
	//--------------------------------------
	if(!empty($_FILES['campus_img']['name'])) { 
	//--------------------------------------
		$path_parts 	= pathinfo($_FILES["campus_img"]["name"]);
		$extension 		= strtolower($path_parts['extension']);
		$img_dir 	= 'uploads/campuses/';
	//--------------------------------------
		$originalImage	= $img_dir.to_seo_url(cleanvars($_POST['campus_name'])).'_'.$campus_id.".".($extension);
		$img_fileName	= to_seo_url(cleanvars($_POST['campus_name'])).'_'.$campus_id.".".($extension);
	//--------------------------------------
	...
	//--------------------------------------
			...
			$mode = '0644'; 
	//--------------------------------------	
			move_uploaded_file($_FILES['campus_img']['tmp_name'],$originalImage);
