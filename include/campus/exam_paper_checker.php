...
if(($_SESSION['userlogininfo']['LOGINTYPE'] == '1' && in_array('12', $_SESSION['userlogininfo']['PERMISSIONS'])) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '12', 'view' => '1'))) {
	require_once(moduleName().'/query.php');
	echo'
	...
	<section role="main" class="content-body">
		...
		<div class="row">
			<div class="col-md-12">';
				...
				elseif($view == 'report'){
					include_once(moduleName().'/report.php');
				}
				else{
	...
	<script type="text/javascript">
		...
		$("#id_class").change(function(){
			...
			$.ajax({  
				...
				data: {
					...
					,"id_exam"		: id_exam
					,"id_campus"	: id_campus
					,"method"		: "attendance_marked_subjects"
				},
				success: function(msg){
					$("#id_subject").html(msg);
				}
		...
		function get_paper_qty() {
			...
			$.ajax({  
				type: "POST",
				url: "include/ajax/get_paper_qty.php",
				data: {
					 "id_campus"	: id_campus
					,"id_exam"		: id_exam
					,"id_class"		: id_class
		...
		$("#id_subject").change(get_paper_qty);
		';
		if(isset($srno)){
			echo'
