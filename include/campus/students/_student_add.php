...
<script>
	...
	$(document).ready(function () {
		$('#std_admissiondate').datepicker({
			autoclose: true,
			todayHighlight: true,
			defaultViewDate: new Date() 
		});
		//
		$('#std_admissiondate').on('show', function (e) {
			if (!$(this).val()) {
	...
	//const allDropdowns = document.querySelectorAll(".select_Box");
	document.querySelectorAll(".select_Box").forEach(function(dropdown) {
...
document.querySelectorAll('.limit').forEach(input => {
    input.addEventListener('change', function () {
        const maxSize = 25 * 1024; 
        const file = this.files[0];

        const errorDisplay = this.parentElement.querySelector('.file-size-error');

        if (file && file.size > maxSize) {
            if (errorDisplay) errorDisplay.textContent = "File must be less than 25KB.";
            this.value = ""; 
        } else {
