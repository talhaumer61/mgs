...
<body class="container">';
    ...
    // DATESHEET
    $condition = array(
                             'select'       =>  'ds.id, ea.id_exam, ss.subject_name, et.type_name, ea.id_subject, ea.id as ea_id, dd.total_marks, c.campus_name, c.campus_logo'
                            ,'join'       	=>  'INNER JOIN '.EXAM_ATTENDANCE.' ea ON ea.id_datesheet = ds.id AND ea.is_publish = 1 AND ea.is_deleted = 0 AND ea.id_subject = '.cleanvars($p_id_subject).'
    ...
    foreach ($DATESHEET AS $key => $val){
        array_push($typeNameArray, $val['type_name']);
    }
    $typeNameArray = implode(', ',$typeNameArray);

    // STUDENTS
    $condition = array(
                             'select'       =>  's.std_id, s.std_name, s.std_fathername, s.std_nic, s.std_rollno, s.std_regno, s.std_photo, sc.class_name, ss.section_name'
                            ,'join'       	=>  'LEFT JOIN '.CLASSES.' sc ON sc.class_id = s.id_class
                                                 LEFT JOIN '.CLASS_SECTIONS.' ss ON ss.section_id = s.id_section'
                            ,'where'        =>  array(
                                                         's.std_status' 	=> 1
                                                        ,'s.is_deleted' 	=> 0
    ...
    $STUDENTS 	= $dblms->getRows(STUDENTS.' s', $condition, $sql);
    ...
    <div class="rollnoslip">
        <table width="100%">
            <tr>
                ...
                <td class="text-center">
                    ...
                    <br>
                    <span style="font-size: 20px;">'.moduleName(false).' ('.$typeNameArray.')</span>
                </td>
            </tr>
        </table>
        ...
        <table width="100%">
            <tr>
                ...
                <td class="text-center">
                    <b>Subject: <u>'.$DATESHEET[0]['subject_name'].'</u></b>
                </td>
                <td class="text-center">
                    <b>Exam Type: <u>'.$typeNameArray.'</u></b>
                </td>
                <td class="text-center">
                ...
                if ($STUDENTS) {
                            ...
                            foreach($DATESHEET AS $key => $val) {
                                ...
                                if ($EXAM_ATTENDANCE_DETAIL['status'] != '2') {
                                    ...
                                    $condition = array(
                                                            ...
                                                            ,'join'         =>  'INNER JOIN '.EXAM_MARKS_DETAILS.' emd ON (em.id = emd.id_setup AND emd.id_std = '.$inval['std_id'].')'
                                                            ,'where'        =>  array(
                                                                                         'em.status' 	    => 1
                                                                                        ,'em.is_publish' 	=> 1
                                                                                        ,'em.is_deleted' 	=> 0
                                                                                        ,'em.id_exam' 	    => cleanvars($val['id_exam'])
                                                                                        ,'em.id_class' 	    => cleanvars($p_id_class)
                                                                                        ,'em.id_section' 	=> cleanvars($p_id_section)
                                                                                        ,'em.id_subject' 	=> cleanvars($p_id_subject)
                                    ...
                                    $EXAM_MARKS = $dblms->getRows(EXAM_MARKS.' em', $condition, $sql);
                    ...
                    <tr>
                        ...
                        for ($i=0; $i < count($DATESHEET); $i++) { 
                            echo'<th class="text-center">'.($noStdWithFullMarks[$i]?$noStdWithFullMarks[$i]:0).'</th>';
                        }
                        echo'
                        <th></th>
                        <th></th>
