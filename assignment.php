...
$srno++;
//-----------------------------------------------------
$sqllmsassign	= $dblms->querylms("SELECT a.assig_title, a.assig_note, a.assig_file, a.open_date, a.close_date, s.subject_name
									FROM ".ASSIGNMENT." a
									INNER JOIN ".CLASS_SUBJECTS." s	ON s.subject_id = a.id_subject
									WHERE a.assig_status = '1' AND a.id_session = '1' AND a.id_class = '".$value_stu['id_class']."' 
									...
									ORDER BY a.assig_id DESC");
$sr = 0;
//-----------------------------------------------------
while($value_assign = mysqli_fetch_array($sqllmsassign)) {
if($value_assign['assig_file']){
    $download = '<a class="btn btn-primary btn-xs" style="width: 90px" href="uploads/resources/'.$value_assign['assig_file'].'"><i class="fa fa-download"></i> Download</a>';
}
else{
    $download = '';
}
...
<tr>
	...
	<td>'.$value_assign['assig_note'].'</td>
	<td class="center">'.date('d, M, Y' , strtotime(cleanvars($value_assign['open_date']))).'</td>
	<td class="center">'.date('d, M, Y' , strtotime(cleanvars($value_assign['close_date']))).'</td>
